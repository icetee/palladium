...
class PalladiumMail extends Mailer {
    ...
    sendMail() {
        ...
        this.setSettings(() => {
            ...
            // Read .mailinglist
            let list = [];
            if (typeof this.mailoptions.bcc === "undefined" || Array.isArray(this.mailoptions.bcc)) {
            ...
            if (fn.hasDuplicates(this.mailoptions.bcc)) {
                let warning = atom.notifications.addWarning("Duplication email address", {
                    ...
                    detail: "Please chek .mailinglist and .mailconfig files",
                    dismissable: true
                });
            } else {
                this.transport();
    ...
    async createConfig() {
        await this.configLoader('mailconfig', '.mailconfig');
    ...
    async createList() {
        await this.configLoader('mailinglist', '.mailinglist');

        if (this.mailinglist.existsSync()) {
            let basename = this.mailinglist.getBaseName();
            atom.notifications.addWarning(`${ basename } already exists.`);

            return false;
        }

        this.mailinglist.write('').then(() => {
            atom.workspace.open(path.normalize(this.mailinglist.path));
        }).catch((error) => {
            atom.notifications.addWarning(error.reason);
        });
